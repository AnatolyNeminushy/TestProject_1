...
public class OpeningAnAccountTests
{
    ...
    /// </summary>
    public async Task OpeningAnAccount_WithExistingDataClient_ReturnsOk(
        string accountType,
        string currency)
    {
        // Arrange
        ...
        var startOperationResponse =
            await restOperations
                .StartOperation("AccountOpen", accessToken);
    ...
    [InlineData(null)]
    [InlineData("&*#&@-")]
    public async Task OpeningAnAccount_WithInvalidOperationCode_ReturnsBadRequest(
    ...
    public async Task OpeningAnAccount_WithInvalidCurrency_ReturnsBadRequest()
    {
        ...
        var body = new List<ParametrOperation>()
        {
            new ParametrOperation {
                Identifier = "AccountType", Value = "Накопительный счёт" },
            new ParametrOperation {
                Identifier = "Currency", Value = "Белорусский Рубль" },
            new ParametrOperation {
